...
import in.dreamlabs.xmlavro.AvroBuilder.{createRecord, findAndCreateNode, unknown}
import in.dreamlabs.xmlavro.Implicits._
import in.dreamlabs.xmlavro.XMLEvents.eleStack
...
class AvroBuilder(schema: Schema,
  ...
  def getDatums(reader: XMLEventReader) = {
      ...
      event getEventType match {
        ...
        case START_ELEMENT =>
          ...
          if (event.name == rootName) {
            rootFound = true
            rootRecord = Some(createRecord(schema))
            currentRecord = rootRecord.get
            currentElement = event name
          }
          if (rootFound) {
            event push()
            val temp = findAndCreateNode(rootRecord.get, event name)
            currentElement = temp._1
            currentRecord = temp._2
            if (event hasAttributes)
              event.attributes foreach {
                case (name, value) =>
                  findAndCreateNode(currentRecord,
                    name,
                    Some(value),
                    attribute = true)
              }
        ...
        case CHARACTERS =>
          if (rootFound && event.hasText)
            findAndCreateNode(currentRecord, event element, Some(event text))
        case END_ELEMENT =>
          if (rootFound) {
            event pop()
